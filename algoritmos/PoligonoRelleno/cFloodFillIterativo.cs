...
namespace algoritmos
{
    public class cFloodFillIterativo
    {
        public static List<cPunto> Rellenar(Bitmap bitmap, cPunto puntoInicio, Color colorRelleno, Color colorBorde)
        {
            ...
            Color colorOriginal = ObtenerColor(rgbValues, puntoInicio.X, puntoInicio.Y, stride);
            ...
            while (pila.Count > 0)
            {
                ...
                Color colorActual = ObtenerColor(rgbValues, p.X, p.Y, stride);

                if (ColoresIguales(colorActual, colorBorde) ||
                    ColoresIguales(colorActual, colorRelleno) ||
